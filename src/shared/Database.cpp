...
void Database::removeDatabaseInterface( DatabaseInterface *db ) {
    delete db;
	WorldServer::getSingleton( ).dbstate--;
    mInterfaces.erase( db );
	char logoutput[50];
	sprintf(logoutput, "DBI: Removed interface [%d]",WorldServer::getSingleton( ).dbstate );
    Log::getSingleton().outString( logoutput );
}
...
DatabaseInterface * Database::createDatabaseInterface( ) {
	...
	WorldServer::getSingleton( ).dbstate++;
	char logoutput[50];
	sprintf(logoutput, "DBI: Added interface [%d]",WorldServer::getSingleton( ).dbstate );
    Log::getSingleton().outString( logoutput );
    return db;
}

void Database::Initialise( const char *host, const char *user, const char *password, const char *database, uint32 port ) {
