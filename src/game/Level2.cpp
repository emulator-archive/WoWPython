...
bool ChatHandler::HandleChangeLevelCommand(uint8* args)
{
	...
	ChangeSelectedNPC(UNIT_FIELD_MAXHEALTH, 100 + 30*Arg, 65535, 1);
	ChangeSelectedNPC(UNIT_FIELD_HEALTH, 100 + 30*Arg, 100 + 30*Arg, 1);
...
bool ChatHandler::HandleRemoveItemCommand(uint8* args)
{
	...
	{
		uint8 buf[256];
		sprintf((char*)buf,"delete from vendord where (vendorGuid='%i') AND (itemGuid='%i')", pUnit->getGUID(), item);
		FillMessageData(&data, 0x09, m_pClient, buf);
		m_pClient->SendMsg( &data );
