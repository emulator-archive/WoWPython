...
void Unit::Update( uint32 p_time ) 
{
    ...
    ////////////DAMAGE LASTS SOME TIME - changed by nothin//////////////////
    if(m_damageDuration > 0)
	{
		...
		if(m_damageTimer == 0)
		{
			...
			if(world.GetCharacter(m_Attacker)!=0)
				DamageAttacker = world.GetCharacter(m_Attacker);
			else
				DamageAttacker = world.GetCreature(m_Attacker);

			if(health-m_damage < 0 || health-m_damage > health)
				m_damageDuration = 0;
			else
				world.mCombatHandler.DealDamage(DamageAttacker, (Unit*)this, m_damage);
		 }
	}
    ///////////////////////////////////////////////////////////////////////
	...
	if(m_pet_state == 2)
	{
		if(m_creatureState != ATTACKING)
		{
			Unit * pUnit = world.GetClosestUnit(this);
			if(pUnit)
			{
				if( closest_dist < 20)
				{
	...
	// AGGRESSIVE MOB CODE BY NOTHIN IT ATTACKS ENEMYS IF THEY GET IN RANGE ( CURRENTLY STATIC 20) SET m_aggressive to 1 to activate
...
void Unit::SendCreateWithTempNpcFlags(UpdateMask *updateMask, GameClient *pClient)
{
    ...
    for( std::list<uint32>::iterator i = mQuestIds.begin( ); i != mQuestIds.end( ); ++ i ) {
        ...
        if (status != 0)
        {
            ...
            else if (pQuest->m_targetGuid == m_guid[0] && status == QUEST_STATUS_AVAILABLE)
            {
...
int Unit::CheckQuestgiverFlag(Character *pPlayer, UpdateMask *unitMask, wowWData * data)
{
    for( std::list<uint32>::iterator i = mQuestIds.begin( ); i != mQuestIds.end( ); ++ i )
    {
        uint32 quest_id = *i;
