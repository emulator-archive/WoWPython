...
void TaxiHandler::HandleMsg( wowWData & recv_data, GameClient *pClient ) {
    wowWData data;
    char f[ 256 ];
    ...
    switch( recv_data.opcode ) {
        case CMSG_TAXINODE_STATUS_QUERY:
            {
                Log::getSingleton( ).outString( "WORLD: Recieved CMSG_TAXINODE_STATUS_QUERY" );
                uint32 guid1, guid2;
                recv_data >> guid1 >> guid2;

                data.Initialise( 9, SMSG_TAXINODE_STATUS );
                data << guid1 << guid2 << uint8( 0 );
                pClient->SendMsg( &data );
                Log::getSingleton( ).outString( "WORLD: Sent SMSG_TAXINODE_STATUS" );
            }break;
        case CMSG_TAXIQUERYAVAILABLENODES:
            {
                Log::getSingleton( ).outString( "WORLD: Recieved CMSG_TAXIQUERYAVAILABLENODES" );
                uint32 guid1, guid2;
                recv_data >> guid1 >> guid2;

                data.Initialise( 32, SMSG_SHOWTAXINODES );
				...
				//data << uint32( 0 );
                pClient->SendMsg( &data );
                Log::getSingleton( ).outString( "WORLD: Sent SMSG_SHOWTAXINODES" );
            }break;
        case CMSG_ACTIVATETAXI:
            {
                ...
                Log::getSingleton( ).outString( "WORLD: Recieved CMSG_ACTIVATETAXI" );
                ...
                data.Initialise( 4, SMSG_ACTIVATETAXIREPLY );
                data << uint32( 0 );
                pClient->SendMsg( &data );
                Log::getSingleton( ).outString( "WORLD: Sent SMSG_ACTIVATETAXIREPLY" );
...
//                pClient->getCurrentChar( )->UpdateObject( );
                ...
                data.Initialise( /*229*/37 + pathnodes.getLength( ) * 4 * 3, SMSG_MONSTER_MOVE );
