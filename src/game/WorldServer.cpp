...
WorldServer::WorldServer()
{
	...
	mClientsConnected=0;
	dbstate = 0;
	motd_size=sizeof("Team Python Default MOTD");
	motd = new uint8[motd_size];
	memcpy(motd,"Team Python Default MOTD",motd_size);
				...
				else if( recv_data.opcode == CMSG_AUTH_SESSION) 
				{
					...
					if (!alreadyconnected) 
					{
						if( account_id < 0 ) 
						{
							...
							//assert(account_id >= 0);  // should be, since we verify in RralmServer first
			...
			if(itr->second->getUpdateValue(UNIT_FIELD_DISPLAYID) == pUnite->getUpdateValue(UNIT_FIELD_DISPLAYID))
			{
				...
				if(itr->second->m_creatureState != ATTACKING)
				{
					...
					if(itr->second->isAlive())
					{
						...
						{
							//if near
							if ( CalcDistance(pUnite, itr->second) < 20)
							{
