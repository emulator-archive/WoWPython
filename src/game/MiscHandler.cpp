...
//                      pClient->getCurrentChar()->SendMessageToSet(&data, false);
						...
						//perhaps make a corpse and transport here?
						...
						if( x != 50000)
						{
							wowWData data;
							//Character *chr = pClient->getCurrentChar();
							pClient->getCurrentChar()->TeleportAck(&data, x+5,y,z);
							pClient->SendMsg(&data);
						...
						uint32 xpt = (uint32)( 200 * pChar->getLevel() * (float) pow(pChar->getLevel(),1.2));
						int32 newxp = xp - xpt;
						if(newxp < 0) 
						{
						...
						uint32 moneyt = (uint32)(200 *  (float) pow(pChar->getLevel(),1.2));
